...
export default function AuthForm({ mode }) {
  ...
  async function submit(event) {
    ...
    try {
      ...
      navigate(roleReturnPath(user.role, location.state?.from) || roleHome[user.role] || '/unauthorized', { replace: true });
    } catch (failure) {
  ...
  return <div className="container auth-page">
    ...
    <section className="auth-form-panel" aria-labelledby="auth-title">
      ...
      <p className="auth-switch">{registering ? 'Already have an account?' : 'New to RailConnect?'} <Link state={location.state} to={registering ? '/login' : '/register'}>{registering ? 'Sign in' : 'Create an account'}</Link></p>
    </section>
  </div>;
}
